...
const HeadersMapping = () => {
  ...
  return (
    <div id="headers-req-specification" className="flex flex-col gap-2">
      <div id="headers-req-buttons" className="flex flex-row gap-3 pb-2">
        <Button icon="fas fa-save" className={"p-0"} />
        <Button label="Add Headers" type="primary_link" onClick={addRow} />
      </div>
      {rows.map((row) => (
        <div
          key={row.id}
